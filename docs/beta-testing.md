...
4. In your browser's extension manager, enable Developer mode and load the `extension` folder unpacked. Copy the connection code from the dashboard into the extension popup. Choose a YouTube video tab, grant access if asked, and select **Use this tab**. The dashboard must report ready media.
