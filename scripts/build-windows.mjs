...
export async function prepareWindowsHelper({ signal, force = false } = {}) {
  ...
  return buildNativeHelper({ source, directory: path.join(root, '.local', 'bin'), name: 'InterludeFocus', extension: '.exe',
    buildKey: 'windows-framework4-v4-uia', signal, force, format: 'pe', executable: true,
    compile: async (destination, buildSignal, snapshot) => {
      try {
        const references = ['UIAutomationClient.dll', 'UIAutomationTypes.dll', 'WindowsBase.dll'].map(name => '/reference:' + path.join(path.dirname(compiler), 'WPF', name));
        await execute(compiler, ['/nologo', '/target:exe', '/optimize+', '/reference:System.Web.Extensions.dll', ...references, `/out:${destination}`, snapshot],
