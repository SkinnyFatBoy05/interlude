...
import { validSignal } from '../extension/task-signals.js';
...
export function createPlatformController({ platform = process.platform, environment = process.env,
  ...
  async function run(mode, { signal, provider = 'codex' } = {}) {
    const result = (code, message) => baseResult(platform, code, provider === 'claude' ? message.replaceAll('Codex', 'Claude').replaceAll('INTERLUDE_CODEX', 'INTERLUDE_CLAUDE') : message);
    const cancelled = () => result('cancelled', 'Desktop return was cancelled.');
    if (!['codex', 'claude'].includes(provider)) return result('invalid_configuration', 'Unknown assistant application.');
    ...
    const bundleId = environment[provider === 'claude' ? 'INTERLUDE_CLAUDE_BUNDLE_ID' : 'INTERLUDE_CODEX_BUNDLE_ID'];
    ...
    if (provider === 'claude') args.push('claude');
    return new Promise(resolve => {
      const finish = (error, stdout) => {
        ...
        try {
          ...
          if (mode === 'observe' && native.observation && validSignal(native.observation)
            && typeof native.observation.available === 'boolean' && /^\d{1,24}$/.test(native.observation.window ?? '')) safe.observation = native.observation;
          return resolve(safe);
  ...
  return {
    focusCodex: ({ maximize = true, signal, provider = 'codex' } = {}) => run(maximize ? 'yes' : 'no', { signal, provider }),
    platformDiagnostics: ({ signal, provider = 'codex' } = {}) => run('status', { signal, provider }),
    observeClaude: ({ signal } = {}) => run('observe', { signal, provider: 'claude' })
  };
}
...
export const observeClaude = controller.observeClaude;
